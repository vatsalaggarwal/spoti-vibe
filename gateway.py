...
def _get_song(m: dict, use_youtube=False) -> Song:
    ...
    return Song(
        title=m["title"], artist=m["artist"], youtube_url=url, youtube_embed_code=embed
    )
...
def query_chain_with_text(query_text: str, k: int = 5) -> List[Song]:
    collection = chroma_client.get_collection("lyrics")
    ...
    song = _get_song(res["metadatas"][0][0])
    songs.append(song)
